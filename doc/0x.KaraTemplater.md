...
This function calls [`maxloop`](#maxloopvar-val-maxmloopvar-val) in order to emit multiple lines;
...
`index` is passed directly to [`maxloop`](#maxloopvar-val-maxmloopvar-val) and controls loop nesting order.
